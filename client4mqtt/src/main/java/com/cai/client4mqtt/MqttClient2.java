...
public class MqttClient2 {
	public static void main(String[] args) {
		try {
			...
			// MQTT设置说明
			try {
				mqtt.setHost("tcp://192.168.142.129:1883");
			} catch (URISyntaxException e) {
				e.printStackTrace();
			}
			mqtt.setClientId("caibojian1"); // 用于设置客户端会话的ID。在setCleanSession(false);被调用时，MQTT服务器利用该ID获得相应的会话。此ID应少于23个字符，默认根据本机地址、端口和时间自动生成
			...
			mqtt.setUserName("cai1");// 服务器认证用户名
			mqtt.setPassword("cai1");// 服务器认证密码
...
//					System.out.println(String.format("debug: " + message, args));
			...
			callbackConnection.connect(new Callback() {
				...
				public void onSuccess(Object value) {
					...
					// 发布消息
					callbackConnection.publish("foo", ("Hello2 ").getBytes(), QoS.EXACTLY_ONCE, true, new Callback() {
